# Legal Checklist Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement the complete Legal Checklist for MemeLaunch using a 2-Hub Architecture (`/privacy` and `/terms`) + global `CookieConsentBanner`.

**Architecture:** A persistent Neobrutalist `CookieConsentBanner` component in layout manages analytics and essential cookie preferences stored in `localStorage` and `cookie-consent` cookie. All legal requirements are organized into two streamlined hubs: Privacy & Data Rights (`/privacy`) and Terms & Acceptable Use (`/terms`), complete with interactive sub-navigation tabs, live cookie preference controls, an interactive GDPR Data Subject Request form, and sub-processor directory.

**Tech Stack:** Next.js 14 (App Router), React 18, TypeScript, Lucide Icons, Tailwind CSS (Vanilla CSS + Neobrutalist classes).

## Global Constraints
- Neobrutalist design aesthetic: dark zinc background (`bg-zinc-950`), high contrast borders (`border-2 border-black` / `border-4 border-black`), yellow (`#ffe600`) and cyan (`text-cyan-400`) accents, brutal shadows (`shadow-brutal`).
- All interactive elements must have unique `id` attributes for e2e testing.
- Must pass `npm run build` cleanly without type errors or missing dependencies.

---

### Task 1: Global Cookie Banner & Preferences Modal

**Files:**
- Create: `components/cookie-consent-banner.tsx`
- Modify: `app/layout.tsx:145-152`

**Interfaces:**
- Produces: `CookieConsentBanner` component rendered at the bottom of the viewport.
- Exports helper function `window.openCookieConsentModal()` to trigger modal re-opening from anywhere.

- [ ] **Step 1: Create `components/cookie-consent-banner.tsx`**
...
- [ ] **Step 2: Add `CookieConsentBanner` to `app/layout.tsx`**

Import and add `<CookieConsentBanner />` inside the `<AuthProvider>` context wrapper in `app/layout.tsx`.
...
### Task 2: Privacy & Data Rights Hub Page (`app/(main)/privacy/page.tsx`)

**Files:**
- Modify: `app/(main)/privacy/page.tsx`

**Interfaces:**
- Consumes: Interactive anchor jump buttons and GDPR Data Subject Request form state.

- [ ] **Step 1: Update `app/(main)/privacy/page.tsx` with complete 4-section layout**

Include:
- Page Header & Meta (`Privacy & Data Rights Hub`)
- Sub-navigation Jump Bar: `[ 1. Privacy Policy | 2. Cookie Policy | 3. GDPR Rights & Requests | 4. DPA & Hosting ]`
- Section 1: Privacy Policy (Account credentials, product submissions, audit logs, retention schedules)
- Section 2: Cookie Policy & Inventory (Cookie details table + live "Manage Cookie Preferences" trigger button calling `window.openCookieConsentModal()`)
- Section 3: GDPR Compliance & Interactive Data Subject Request Form (Options for Data Access/Export, Account Erasure, Rectification; state handling with success feedback)
- Section 4: Data Processing Agreement & Sub-Processor Directory (Table showing InsForge/Postgres, Cloud Storage, Vercel, PostHog with processing purpose and locations)
...
### Task 3: Terms of Service & Acceptable Use Hub Page (`app/(main)/terms/page.tsx`)

**Files:**
- Modify: `app/(main)/terms/page.tsx`

- [ ] **Step 1: Update `app/(main)/terms/page.tsx` with complete 4-section layout**

Include:
- Page Header & Meta (`Terms of Service & Guidelines Hub`)
- Sub-navigation Jump Bar: `[ 1. Terms of Service | 2. Acceptable Use Policy | 3. Points Rules | 4. DMCA Copyright ]`
- Section 1: Terms of Service (Acceptance, user accounts, license grants, warranty disclaimers, liability limits)
- Section 2: Acceptable Use Policy (Permitted software launches, prohibited content like scams/malware/hate speech, anti-fraud enforcement)
- Section 3: Point System & Gamification Terms (15 points cost per submission, non-monetary value, ban policy for automated exploitation)
- Section 4: DMCA Copyright & Content Removal (DMCA takedown notice requirements, support email contact)
...
### Task 4: Footer Navigation & Sitemap Integration

**Files:**
- Modify: `components/footer.tsx:88 text`
- Modify: `app/sitemap.ts:1-40`

- [ ] **Step 1: Update `components/footer.tsx`**

Update Legal section in `Footer` to render:
- `Terms & Guidelines` -> `/terms`
- `Privacy & Data Rights` -> `/privacy`
- `Cookie Preferences` -> button triggering `window.openCookieConsentModal()`

- [ ] **Step 2: Update `app/sitemap.ts`**

Ensure `/privacy` and `/terms` are listed in the sitemap array with proper canonical URLs, priority `0.6`, and `changeFrequency: 'monthly'`.

- [ ] **Step 3: Verify build**
Run: `npm run build`
Expected: Successful Next.js build without warnings.

---

### Task 5: End-to-End Verification & Audit
...
Run: `npm run build`
Expected: Clean compilation, all static routes built successfully.
