# This Week's Products Showcase Section Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a high-impact, neo-brutalist "This Week's Products" showcase section on the home feed (`app/(main)/home-feed.tsx`) highlighting the top-voted product launches from the last 7 days with live rank badges (`🥇 #1`, `🥈 #2`, `🥉 #3`), reaction counters, and a cycle timer.

**Architecture:** Create a modular client component `ThisWeeksProducts` (`components/feed/this-weeks-products.tsx`) that receives the list of launches, filters launches created within the past 7 days (or falls back to top recent launches if fewer than 3 exist), ranks them by reaction counts, and renders a responsive mobile-swipeable snap carousel / desktop grid showcase directly beneath the Hero section on the homepage.

**Tech Stack:** Next.js 15 (React 19), Tailwind CSS v4, Lucide React icons, InsForge SDK / Image handling.

## Global Constraints

- **Design Tokens & Aesthetic**: Neo-brutalist dark mode with `#ffe600` accents, `border-2 border-black` / `border-4 border-black`, `shadow-brutal` and `shadow-brutal-sm`, and Outfit/Inter/Impact fonts matching existing design system (`context/ui-context.md`).
- **Time Threshold**: 7 days (`created_at >= Date.now() - 7 * 24 * 60 * 60 * 1000`).
- **Strict Verification**: Build must compile with zero errors (`npm run build` or `npx tsc --noEmit`).

---

### Task 1: Create `ThisWeeksProducts` Showcase Component

**Files:**
- Create: `components/feed/this-weeks-products.tsx`

**Interfaces:**
- Consumes: `Launch` interface from `@/components/feed/meme-card`
- Produces: `ThisWeeksProducts` React Component taking `{ launches: Launch[] }`

- [ ] **Step 1: Create `components/feed/this-weeks-products.tsx` with filtering, cycle countdown timer, rank badges, and responsive carousel/grid UI**
...
import { Flame, Sparkles, Clock, Trophy, ChevronRight, Rocket, ArrowRight } from 'lucide-react';
...
- [ ] **Step 2: Verify component TypeScript definitions and compilation**
...
```bash
git add components/feed/this-weeks-products.tsx
git commit -m "feat: add ThisWeeksProducts showcase component"
```

---

### Task 2: Integrate `ThisWeeksProducts` in `app/(main)/home-feed.tsx`

**Files:**
- Modify: `app/(main)/home-feed.tsx`

- [ ] **Step 1: Import and place `ThisWeeksProducts` component in `home-feed.tsx`**

In `app/(main)/home-feed.tsx`:
1. Import `ThisWeeksProducts` at top of file:
   `import { ThisWeeksProducts } from '@/components/feed/this-weeks-products';`
2. Insert `<ThisWeeksProducts launches={launches} />` directly below the `<section className="relative overflow-hidden rounded-3xl...` Hero section and above the `{/* Feed Filter & Search Row */}` section.

- [ ] **Step 2: Verify home feed rendering and full project compilation**
...
git commit -m "feat: integrate ThisWeek's Products showcase on home feed"
```
...
- Run Next.js static build check: `npm run build`
- Ensure zero TypeScript or linting errors.

### Manual Verification
- Open `http://localhost:3000` in browser.
- Verify the "THIS WEEK'S PRODUCTS" section appears directly under the Hero section.
- Verify rank badges (`🥇 #1`, `🥈 #2`, `🥉 #3`) render correctly on top-voted launches.
- Verify clicking any card opens the product detail page `/products/[productName]`.
