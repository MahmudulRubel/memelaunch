...
export default function ProfilePage() {
  ...
  const fetchProfileAndLaunches = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'An unexpected error occurred while fetching details.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // 3. Refresh Auth provider state so header updates
      await refreshUser();
  ...
  const handleDownloadShareCard = async () => {
    ...
    try {
      ...
      // Card Dimensions: 800 x 450 (standard 16:9 slot)
      canvas.width = 800;
      canvas.height = 450;

      // 1. Draw Gradient Background
      const bgGrad = ctx.createLinearGradient(0, 0, 800, 450);
      bgGrad.addColorStop(0, '#09090b'); // bg-zinc-950
      bgGrad.addColorStop(0.5, '#18181b'); // bg-zinc-900
      bgGrad.addColorStop(1, '#020617'); // slate-950
      ctx.fillStyle = bgGrad;
      ctx.fillRect(0, 0, 800, 450);

      // 2. Draw border
      ctx.strokeStyle = 'rgba(163, 230, 53, 0.2)'; // lime-400/20
      ctx.lineWidth = 4;
      ...
      // Helper function to draw profile image safely
      const drawProfileAvatar = (): Promise<void> => {
        return new Promise((resolve) => {
          ...
          const img = new Image();
          img.crossOrigin = 'anonymous'; // Avoid taint
          img.onload = () => {
            try {
              ctx.save();
              ctx.beginPath();
              ...
              console.error('CORS blocked avatar drawing on canvas, using fallback');
              drawFallbackAvatar();
            }
            resolve();
          };
      ...
      // 4. Stats section (horizontal row)
      const drawStat = (label: string, value: number, x: number) => {
        // Stats container
        ctx.fillStyle = 'rgba(24, 24, 27, 0.6)'; // zinc-900
      ...
      // 5. Watermarks
      ctx.textAlign = 'left';
      ctx.fillStyle = 'rgba(163, 230, 53, 0.4)'; // lime-400/40
      ctx.font = 'bold 20px Impact, sans-serif';
      ctx.fillText('MEMELAUNCH', 50, 380);

      ctx.fillStyle = 'rgba(161, 161, 170, 0.4)'; // zinc-400/40
      ...
      alert('Failed to generate sharing card. Please try again.');
    } finally {
      setIsGeneratingCard(false);
    }
  };

  // Pre-fill X / Twitter tweet
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-300">
      {/* Hidden canvas for card compiler */}
              ...
              {isEditing ? (
                <div className="space-y-3 max-w-sm">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-3 py-1.5 bg-zinc-950 border border-zinc-800 rounded-xl text-xs focus:outline-none focus:border-lime-500 text-zinc-300"
                  />
                </div>
              ) : (
                <>
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5">
                    <h1 className="font-extrabold text-2xl md:text-3xl text-zinc-50 group-hover:text-lime-400 transition-colors">
                      {profile.name || 'Founder'}
                    </h1>
                    
                    {isOwnProfile && (
                      <span className="px-2 py-0.5 rounded-md bg-zinc-800 border border-zinc-700/60 text-[9px] font-mono text-zinc-400 tracking-wider uppercase font-bold">
                        YOU
                      </span>
                    )}
                  </div>
              ...
              isEditing ? (
                <>
                  <button
                    ...
                    disabled={isSaving}
                    className="px-4 py-2 bg-lime-400 hover:bg-lime-300 disabled:bg-zinc-800 disabled:text-zinc-650 text-zinc-950 font-extrabold text-xs uppercase tracking-wider rounded-xl transition-all shadow-[0_0_15px_rgba(163,230,53,0.15)] flex items-center gap-1.5 cursor-pointer disabled:cursor-not-allowed"
                  >
                    {isSaving ? (
                ...
                <button
                  ...
                  className="px-4 py-2 bg-zinc-900 hover:bg-zinc-800/80 border border-zinc-800 text-zinc-300 font-bold text-xs uppercase tracking-wider rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
                >
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div className="p-5 bg-zinc-900/30 border border-zinc-800/80 rounded-2xl flex flex-col justify-between h-32 relative overflow-hidden group">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-zinc-500 font-mono">memes</span>
          </div>
        </div>
        ...
        <div className="p-5 bg-zinc-900/30 border border-zinc-800/80 rounded-2xl flex flex-col justify-between h-32 relative overflow-hidden group">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-extrabold text-rose-450 font-mono">
