...
export default function TemplatesPage() {
  ...
  // Fetch templates and launches using templates
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        // Fetch all templates
        const { data: templatesData, error: templatesError } = await insforge.database
          .from('templates')
          .select('*')
        ...
        // Fetch approved launches that used a template
        const { data: launchesData, error: launchesError } = await insforge.database
          .from('launches')
        ...
        if (launchesError) {
          console.error('Failed to load launches using templates:', launchesError);
        }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-4">
        ...
        <p className="text-zinc-400 font-mono text-sm">Fetching meme templates...</p>
      </div>
    );
  }
  ...
  if (errorMsg) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 bg-zinc-900/20 border border-zinc-800/60 rounded-3xl text-center space-y-4 max-w-xl mx-auto shadow-2xl">
        <AlertCircle className="h-12 w-12 text-rose-500" />
        <h3 className="text-xl font-bold text-zinc-200">Failed to load templates</h3>
        <p className="text-zinc-400 text-sm">{errorMsg}</p>
        <button
          onClick={() => window.location.reload()}
          className="px-6 py-2 bg-zinc-900 border border-zinc-850 text-zinc-200 hover:text-zinc-50 rounded-xl transition-colors font-bold text-xs uppercase"
        >
          Try Again
        </button>
      </div>
    ...
    <div className="space-y-12 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center mt-6">
            ...
            <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
              ...
              <div className="pt-2 flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                ...
                <button
                  onClick={() => setPreviewTemplate(weeklyPick)}
                  className="w-full sm:w-auto px-6 py-3 bg-zinc-900/60 hover:bg-zinc-800/80 border border-zinc-800 text-zinc-355 hover:text-zinc-100 font-bold text-xs uppercase tracking-wider rounded-xl transition-all"
                >
                  View Large Template
                </button>
              </div>
      ...
      <section className="space-y-6">
        ...
        {regularTemplates.length === 0 ? (
          <div className="p-12 bg-zinc-900/20 border border-zinc-850 rounded-3xl text-center text-zinc-500 font-mono text-sm">
                    ...
                    <div className="absolute inset-0 bg-zinc-950/80 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center p-6 space-y-4">
                      ...
                      <div className="flex flex-col gap-2 w-full pt-2">
                        ...
                        <button
                          ...
                          className="w-full py-2 bg-zinc-900/80 hover:bg-zinc-805 text-zinc-300 hover:text-zinc-100 border border-zinc-800 rounded-lg font-bold text-[10px] uppercase tracking-wider transition-all"
                        >
                          Quick Preview
                        </button>
                      </div>
                      ...
                      <div className="space-y-2 pt-2 border-t border-zinc-850/60">
                        ...
                        <div className="flex -space-x-2 overflow-hidden items-center">
                          ...
                          {tplLaunches.length > 4 && (
                            <span className="pl-3 text-[10px] font-mono text-zinc-505">
